...
ammo.types = {}
ammo.types[1] = "default"
ammo.types[2] = "armorpierce"
ammo.types[3] = "concussion"
ammo.types[4] = "corrosive"
ammo.types[5] = "dirty"
ammo.types[6] = "he"
ammo.types[7] = "hollowpoint"
ammo.types[8] = "incendiary"
ammo.types[9] = "plasma"
ammo.types[10] = "shock"
ammo.types[11] = "tracker"
ammo.types[12] = "tranquilizer"
...
function ammo.incendiary(bullet)

	bullet.Damage = bullet.Damage * IncenDamageMult
	bullet.Force = bullet.Force * 0.85

	bullet.Callback = function(client, trace, dmg)
		if IsValid(client) and client:IsPlayer() then
			if trace.Hit and not trace.HitSky and not IsValid(trace.Entity) then
			...
			elseif trace.Hit and IsValid(trace.Entity) and not trace.HitWorld then
				local chance = math.random(1,100)
				if chance >= (100 - IncenIgniteChance) then
...
function ammo.armorpierce(bullet)

	bullet.Damage = bullet.Damage * ArmorPierceDamageMult
	...
	bullet.Callback = function(client, trace, dmg)
		if IsValid(client) and client:IsPlayer() then
			...
			if IsValid(ent) and ent:IsPlayer() then
				...
				if armor >= 50 then
					local chance = math.random(1,100)
					if chance >= (100 - ArmorPierceChance) then
...
function ammo.plasma(bullet)

	bullet.Damage = bullet.Damage * PlasmaDamageMult
	...
	bullet.Callback = function(client, trace, dmg)
		if IsValid(client) and client:IsPlayer() then
			...
			if IsValid(ent) and (ent:IsPlayer() or ent:IsNPC()) then
				if ent:Health() <= (ent:GetMaxHealth() * ((ent:IsPlayer() and PlasmaDissolveThreshold or 75) / 100)) then
					local chance = math.random(1,100)
					dmg:SetDamage(ent:Health() + 500)
					if chance >= (100 - PlasmaDissolveChance) then

						if ent:IsPlayer() and IsValid(ent:GetRagdollEntity()) and not ent:GetRagdollEntity().dust then
						...
						elseif IsValid(ent) and ent:IsNPC() and not ent.dust and not blacklist[ent:GetClass()] then
							ent.dust = true
			...
			if trace.Hit and not trace.HitSky and trace.HitWorld then
				local chance = math.random(1,100)
				--print("pernicious burn")
				if chance >= 50 then
...
function ammo.hollowpoint(bullet)
	bullet.Callback = function(client, trace, dmg)
		if IsValid(client) and client:IsPlayer() then
			local ent = trace.Entity
			if IsValid(ent) then
				if ent:IsPlayer() and ent:Armor() > 0 then
					dmg:SetDamage(dmg:GetDamage() * HollowPointDamageReduction)
				elseif ent:IsPlayer() and ent:Armor() <= 0 then
					dmg:SetDamage(dmg:GetDamage() * HollowPointDamageMult)
...
function ammo.concussion(bullet)
	bullet.Damage = bullet.Damage * ConcussionDamageMult
		...
		if IsValid(client) and client:IsPlayer() then
			...
			if IsValid(player) and player:IsPlayer() then
				if trace.HitGroup == HITGROUP_LEFTARM or trace.HitGroup == HITGROUP_RIGHTARM then
					local chance = (math.random(1,100) >= (100 - ConcussionDropWeaponChance))
					if chance then
						player:DropCurwep()
					end
				elseif trace.HitGroup == HITGROUP_LEFTLEG or trace.HitGroup == HITGROUP_RIGHTLEG then
					local chance = (math.random(1,100) >= (100 - ConcussionCrippleChance))
					if chance then
						player:Cripple()
						timer.Simple(3, function() player:StandUp() end)
					end
				end
			end
		end
...
function ammo.he(bullet)
	bullet.Damage = bullet.Damage * HEDamageMult

	bullet.Callback = function(client, trace, dmg)
		if IsValid(client) and client:IsPlayer() then
			local chance = math.random(1,100)
			if chance >= (100 - HEExplosionChance) and !trace.HitSky then
				net.Start("goboom")
				net.WriteVector(trace.HitPos)
				net.Broadcast()

				util.BlastDamage(client:GetActiveWeapon(), client, trace.HitPos, HEExplosionRadius, HEExplosionBonusDamage)
			end
		end
	end
end

function ammo.dirty(bullet)
	bullet.Damage = bullet.Damage * DirtyDamageMult
end

function ammo.tracker(bullet)
	bullet.Damage = 0
	bullet.Force = 0
	bullet.TracerName = "AR2Tracer"
		...
		if IsValid(client) and client:IsPlayer() then
			...
			if IsValid(ent) then
				if !IsValid(ent.dlight) then
					...
					ent.dlight:SetKeyValue("_light", string.format("%s %s %s 255", TrackerColor.r, TrackerColor.g, TrackerColor.b))
					ent.dlight:SetKeyValue("distance", 250)
					ent.dlight:SetKeyValue("brightness", 3.5)
					ent.dlight:Spawn()
					ent.dlight:Activate()
					timer.Create("dlightdie"..ent:EntIndex(), TrackerLifetime, 1, function() SafeRemoveEntity(ent.dlight) end)
				else
					timer.Create("dlightdie"..ent:EntIndex(), TrackerLifetime, 1, function() SafeRemoveEntity(ent.dlight) end)
				end
			end
...
function ammo.tranquilizer(bullet)
	bullet.Damage = 0
	bullet.Force = 0
	...
	bullet.Callback = function(client, trace, dmg)
		if IsValid(client) and client:IsPlayer() then
			local ent = trace.Entity
			if IsValid(ent) and ent:IsPlayer() and not ent:GetNWBool("tranqed", false) and not ent:HasGodMode() then
				ent:SetNWBool("tranqed", true)
				ent:SetDSP(132)
				ent:FadeScreen(5, TranqDartDuration, 10)
				ent:ConCommand("soundfade 100 "..TranqDartDuration.." 7 5")

				timer.Simple(2, function()
					ent:Cripple(10+TranqDartDuration, true)
					net.Start("tranqed")
					net.Send(ent)
				end)

				timer.Simple(13 + TranqDartDuration, function()
					ent:StandUp()
					ent:SetDSP(0)
...
function ammo.corrosive(bullet)
		...
		if IsValid(client) and client:IsPlayer() then
			...
			if IsValid(ent) and ent:IsPlayer() or ent:IsNPC() then
				...
				if chance >= 20 then
					...
					net.Broadcast()
					if not timer.Exists("corrode" .. ind) then
						ent:Corrode(client)
					end
				end
...
function ammo.shock(bullet)
	bullet.Damage = bullet.Damage * ShockDamageMult
	...
	bullet.Callback = function(client, trace, dmg)
		if IsValid(client) and client:IsPlayer() then
			...
			if IsValid(ent) then
				...
				if chance >= (100 - ShockArcChance) and (ent:IsNPC() or ent:IsPlayer()) then
